...
function draw(flags){
    ...
    if (flags.embed){
        ...
        loadImage(flags.embed).then((image) => {
          // Check embedPosition if it contains "x" and separate it
          if (flags.embedPosition.includes("x")){
            const embedPosition = flags.embedPosition.split("x")
            const embedX = parseInt(embedPosition[0]);
            const embedY = parseInt(embedPosition[1]);
            ctx.drawImage(image, embedX, embedY)
          } else if (flags.embedPosition == "lowerright") {
